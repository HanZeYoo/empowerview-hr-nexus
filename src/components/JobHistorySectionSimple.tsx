...
const JobHistorySectionSimple: React.FC<JobHistorySectionProps> = ({
  employeeNumber,
  employeeName,
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .insert({
          ...
          effdate: format(newJobHistory.effdate, 'yyyy-MM-dd'),
      ...
      console.error("Error adding job history:", error.message);
      toast({
        variant: "destructive",
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const { error } = await supabase
        .from("jobhistory")
        .update({
          jobcode: editingJobHistory.jobcode,
          deptcode: editingJobHistory.deptcode,
          effdate: format(new Date(editingJobHistory.effdate), 'yyyy-MM-dd'),
          salary: editingJobHistory.salary
        })
        .eq("empno", employeeNumber)
        .eq("jobcode", jobHistories[editIndex].jobcode)
        .eq("effdate", format(new Date(jobHistories[editIndex].effdate), 'yyyy-MM-dd'));
      ...
      console.error("Error updating job history:", error.message);
      toast({
        variant: "destructive",
        title: "Failed to update job history",
        description: error.message,
      });
    }
  };
  ...
  const handleRemoveJobHistory = async (index: number) => {
    try {
      ...
      const { error } = await supabase
        ...
        .delete()
        .eq("empno", employeeNumber)
        .eq("jobcode", historyToRemove.jobcode)
        .eq("effdate", format(new Date(historyToRemove.effdate), 'yyyy-MM-dd'));
      ...
      toast({
        ...
        description: "Job history record has been successfully removed"
      });
    } catch (error: any) {
      console.error("Error removing job history:", error.message);
      toast({
        variant: "destructive",
        title: "Failed to delete job history",
        description: error.message,
      });
    }
  };
...
export default JobHistorySectionSimple;
